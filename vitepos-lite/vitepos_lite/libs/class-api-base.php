	...
	abstract class API_Base {
		...
		public function check_file_type() {
			...
			foreach ( $_FILES as $file ) {
				...
				$names = $this->flatten_file_field( $file['name'] );
				$tmps  = $this->flatten_file_field( isset( $file['tmp_name'] ) ? $file['tmp_name'] : '' );
				foreach ( $names as $key => $name ) {
					if ( '' === $name || null === $name ) {
						continue;
					}
					$tmp_name = isset( $tmps[ $key ] ) ? $tmps[ $key ] : '';
					$validate = wp_check_filetype_and_ext( $tmp_name, $name, $allowed_mimes );
		...
		/**
		 * Flatten a possibly-nested $_FILES sub-array (name or tmp_name) into a
		 * flat, path-keyed list so scalar and array file fields validate alike.
		 * name and tmp_name share the same structure, so identical keys align them.
		 *
		 * @param mixed  $value  The $_FILES sub-array value (string or nested array).
		 * @param string $prefix Internal path accumulator.
		 *
		 * @return array Flat map of path => scalar value.
		 */
		private function flatten_file_field( $value, $prefix = '' ) {
			$out = array();
			if ( is_array( $value ) ) {
				foreach ( $value as $k => $v ) {
					$child = ( '' === $prefix ) ? (string) $k : $prefix . '.' . $k;
					$out   = $out + $this->flatten_file_field( $v, $child );
				}
			} else {
				$out[ $prefix ] = $value;
			}
