...
class Pos_User_Api extends API_Base {
	...
	/**
	 * Per-target authorization for user-management routes (CWE-862).
	 *
	 * A flat capability such as `user-edit` / `change-any-user-pass` is
	 * caller-scoped: it establishes that the caller may manage SOME users, never
	 * whom. The default Outlet Manager role holds those capabilities, so without
	 * this per-target check an Outlet Manager could edit the administrator
	 * (user ID 1) — hijacking the email (password-reset takeover) or stripping
	 * the role (lockout). Decides whether the current caller may manage the given
	 * target user.
	 *
	 * @param \WP_User $user_data The target user being managed.
	 *
	 * @return bool True when the caller is allowed to manage this target.
	 */
	private function can_manage_target_user( $user_data ) {
		if ( empty( $user_data->ID ) ) {
			return false;
		}
		$target_id  = absint( $user_data->ID );
		$current_id = absint( $this->get_current_user_id() );
		if ( $target_id === $current_id ) {
		...
		$agent_roles  = Mapbd_Pos_Role::get_agent_roles();
		$target_roles = (array) $user_data->roles;
		if ( empty( $target_roles ) || user_can( $target_id, 'edit_users' ) ) {
			return false;
		}
		foreach ( $target_roles as $target_role ) {
			if ( ! in_array( $target_role, $agent_roles, true ) ) {
	...
	public function create_user() {
		if ( ! empty( $this->payload ) ) {
			...
			if ( ! empty( $old_cus ) ) {
				...
				if ( ! $this->can_manage_target_user( $old_cus ) ) {
					$this->add_error( 'You are not allowed to edit this user.' );
					$this->response->set_response( false, 'You do not have permission to do this' );
					return $this->response->get_response();
				}
