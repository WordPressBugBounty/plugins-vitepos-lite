...
class Appsbd_Related_App extends Vitepos_Module {
	...
	/**
	 * The generate plugin activation url is generated by appsbd
	 *
	 * @param mixed $plugin It is plugin_file param.
	 *
	 * @return false|string
	 */
	public function generate_plugin_activation_url( $plugin ) {
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return false;
		}
		$url = admin_url( 'plugins.php?action=activate' );
		return str_replace( '&amp;', '&', wp_nonce_url( $url, 'activate-plugin_' . $plugin ) ) . '&plugin=' . urlencode( $plugin );
	}

	/**
	 * The generate plugin install url is generated by appsbd
	 *
	 * @param mixed $plugin It is plugin param.
	 *
	 * @return false|string
	 */
	public function generate_plugin_install_url( $plugin ) {
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return false;
		}
	...
	public function next_actions( $addon ) {
		...
		if ( ! empty( $addon->pro_package ) || ! empty( $addon->lite_package ) ) {
			if ( ! $this->is_exist_plugin( $addon->pro_package ) && ! $this->is_exist_plugin( $addon->lite_package ) ) {
				...
				$obj->btn_icon     = 'vco vco-check-circle';
				$obj->next_actn    = '';
				$addons[]          = $obj;
				...
				if ( ! appsbd_is_activated_plugin( $addon->lite_package ) ) {
					...
					$obj->activate_link = $this->generate_plugin_activation_url( $addon->lite_package );
					$addons[]           = $obj;
	...
	public function get_addons() {
		...
		foreach ( $addons as &$addon ) {
			$addon->install_link = '';
			if ( ! $this->is_exist_plugin( $addon->pro_package ) && ! $this->is_exist_plugin( $addon->lite_package ) ) {
				$addon->install_link = esc_url( network_admin_url( 'plugin-install.php?tab=plugin-information&plugin=' . basename( $addon->lite_package ) ) );
			}
