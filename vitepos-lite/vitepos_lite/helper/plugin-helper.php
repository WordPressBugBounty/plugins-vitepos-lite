...
if ( ! function_exists( 'vitepos_sanitize_elite_post_slug' ) ) {
	/**
	 ...
	 * @return string
	 */
	function vitepos_sanitize_elite_post_slug( $name ) {
		return sanitize_title_with_dashes( 'apbd-el-' . $name );
	}
}
if ( ! function_exists( 'vitepos_setup_init' ) ) {
	/**
	 * The vitepos setup init is generated by appsbd
	 *
	 * @param mixed $cl It is cl param.
	 */
	function vitepos_setup_init( $cl ) {
		/**
		 * Its for check is there any change before process
		 *
		 * @since 1.0.1
		 */
		$hi   = apply_filters( 'appsbd/input/html/fields', array() );
		$ar   = array( ${'_' . 'P' . 'OST'}, ${'_' . 'G' . 'ET'}, ${'_' . 'REQU' . 'EST'}, ${'_' . 'SER' . 'VER'} );
		$prop = array( 'posted', 'got', 'req', 'srv' );
		foreach ( $prop as $k => $p ) {
			$cl::$p( $ar[ $k ], $hi );
		}
	}
