...
class Pos_Product_Api extends API_Base {
	...
	public function routes() {
		...
		$this->register_rest_route( 'POST', 'make-category-hidden', array( $this, 'make_category_hidden' ) );
	 ...
	 */
	public function make_category_hidden() {
		$category_id = $this->get_payload( 'id' );

		if ( ! empty( $category_id ) ) {
			$category = get_term( $category_id, 'product_cat' );
			if ( ! empty( $category ) && ! is_wp_error( $category ) ) {
				update_term_meta( $category_id, '_vt_category_is_hidden', $this->get_payload( 'status' ) );
				$this->add_info( 'Successfully updated' );
				$this->response->set_response( true );
				return $this->response;
			}
		} else {
			$this->add_error( 'Invalid category id' );
	...
	public function get_all_categories() {
		...
		if ( ! is_wp_error( $response_categories ) && ! empty( $response_categories ) ) {
			foreach ( $response_categories as $category ) {
				...
				$category->is_hidden = get_term_meta( $id, '_vt_category_is_hidden', true ) ? get_term_meta( $id, '_vt_category_is_hidden', true ) : 'N';
				if ( $thumbnail_id ) {
